...
- Path: `components/job-details/CompanyResearch.tsx` — **always renders its empty state** as of feature 12, per build-plan's explicit instruction. Heading (`Building2` icon in an accent circle + "Company Research") with a `Button variant="primary"` "Research Company" (`Search` icon) top-right — **present but inert, no `onClick`**, same precedent as feature 05's "Generate Resume from Profile" button (built inert, wired two features later). Feature 13 (Company Research Agent) is what wires this one. Below a `border-t` divider: centered empty state (`Building2` icon in a gray circle, "No research yet", a `max-w-md` explanation naming the real company).
