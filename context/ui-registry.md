...
- **`RecentActivity.tsx`** — 5 hardcoded mock entries matching the design's exact copy/timestamps. Dot ring/inner colors were read directly from the design's pixel values (`accent` #7C5CFC, `info` #61A8FF, `success-alt` #00BC7D — the same three tokens `ui-tokens.md`'s Activity Dots table already defines) rather than inferred from a type→color rule: the design assigns purple/blue/green per-entry in a way that doesn't cleanly map to "job found" vs "researched" (row 1 "Found 8 jobs" is purple, row 3/5 "Found ..." are green), so this mock hardcodes the exact per-row color the design shows instead of inventing a rule the design doesn't actually follow. Dot: `h-4 w-4` ring (`border-2 border-surface` + token bg) containing a `h-2 w-2` inner circle, divided rows via `divide-y divide-border`.
