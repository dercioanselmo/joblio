...
## Dashboard Page (`context/designs/dashboard.png`, built 2026-08-11, mock data only per build-plan feature 14)

Page path: `app/(app)/dashboard/page.tsx` — plain Server Component, `min-h-screen px-8 py-10` outer / `mx-auto flex max-w-[1440px] flex-col gap-6` inner (same layout tokens as `/find-jobs`). Composes `StatsBar`, then a `grid lg:grid-cols-2` row with `RecentActivity` + a Company Research Activity card, then a second `grid lg:grid-cols-2` row with Jobs Found Over Time + Match Score Distribution cards. All data is a hardcoded mock array per component/page — no DB or PostHog queries yet (features 15-17).

- **New approved dependency: `recharts`** — added to `code-standards.md`'s approved list. `build-plan.md` feature 17 explicitly calls for "all three charts rendered with recharts" and `ui-tokens.md`'s Dashboard Chart Colors section (dashed grid, gradient area fill, per-chart stroke colors) already assumes a charting library with that exact feature set — not a guess, the existing docs already committed to this choice, they just hadn't been acted on yet.
- **`StatsBar.tsx`** — 4 mock `StatCardData` entries rendered inline (no separate `StatCard.tsx` — four near-identical cards differing only in copy didn't warrant a second file/prop-drilling layer). Card: `rounded-2xl border border-border bg-surface p-6 shadow`; value `text-[30px] font-semibold leading-9` per `ui-tokens.md`'s Stat Number spec; trend badge only rendered for the two cards that have one (Total Jobs Found, Avg. Match Rate) — `rounded-sm bg-success-lightest px-2 py-0.5 text-xs font-medium text-success-darker`, exactly the Trend Badge token; the other two cards (Companies Researched, Jobs This Week) show plain `text-text-muted` subtitle text with no badge, matching the design pixel-for-pixel (verified — no green pill on those two).
- **`RecentActivity.tsx`** — feature 14 mock version had 5 hardcoded entries with per-row dot colors read from the design's pixel values (3 colors, no consistent type→color rule). **Superseded in feature 16 (real data, 2026-08-11):** now `{ entries: ActivityItem[] }`, sourced from `lib/activity.ts`'s `buildRecentActivity()`. Dot color is now strictly type-based per `build-plan.md`'s literal "info blue, success green" spec — `ActivityDot` is a 2-value union (`info`/`success`), the mock's third `accent` variant was dropped. Real empty state ("No activity yet. Search for jobs or research a company to get started.") added per `ui-rules.md`. Dot: `h-4 w-4` ring (`border-2 border-surface` + token bg) containing a `h-2 w-2` inner circle, divided rows via `divide-y divide-border` — markup unchanged from feature 14, only the data source and color rule changed.
- **`lib/activity.ts`** (feature 16) — `buildRecentActivity(runs, researchedJobs, limit=5)`, a pure function (no DB access) merging two already-fetched row sets, sorting by timestamp descending, and slicing to `limit`. Search entries come from `agent_runs` (`status='completed' AND job_title_searched IS NOT NULL`, timestamp = `completed_at ?? started_at`); research entries come from `jobs` (`company_research IS NOT NULL`, timestamp = `updated_at`). **`updated_at` stands in for a "researched at" timestamp that doesn't otherwise exist in the schema** — `company_research` is a bare jsonb column with no companion timestamp, and the research route never links its `agent_runs` row back to which job it researched, so the jobs table's own generic `updated_at` trigger (already bumped by the research route's `UPDATE jobs SET company_research = ...`) is the only real signal available, not a new column.
- **`CompanyResearchChart.tsx` / `JobsFoundChart.tsx` / `MatchScoreChart.tsx`** — each its own `"use client"` file (recharts needs a client boundary), one exported component per file per `code-standards.md`. All three: `ResponsiveContainer` height 280, `CartesianGrid strokeDasharray="4 4" stroke="#E7EAF3"` (horizontal only), axis ticks `12px #9CA3AF` with no axis line/tick marks on the Y axis, matching `ui-tokens.md`'s Dashboard Chart Colors section exactly (grid `1px dashed #E7EAF3`, axis labels `#9CA3AF` 12px). Y-axis domains/ticks are hardcoded to match the design's exact scale rather than left to recharts' auto-generated ticks: Company Research `[0,3,6,9,12]`, Jobs Found and Match Score both `[0,25,50,75,100]`.
  - `CompanyResearchChart`: `BarChart`, fill `#61A8FF`, `radius={[4,4,0,0]}`.
  - `JobsFoundChart`: `AreaChart` with `type="monotone"`, stroke `#7C5CFC` width 3, fill is a `linearGradient` from `rgba(124,92,252,0.2)` to transparent — matches `ui-tokens.md`'s documented gradient exactly.
  - `MatchScoreChart`: `BarChart`, fill `#10B981`, categories are the fixed 5 score-range buckets from `build-plan.md` (`50-60% ... 90-100%`).
  - **Bug fix (2026-08-11):** none of the three originally shipped with a `<Tooltip>` — hovering a bar/point showed nothing. Added to all three: `contentStyle`/`labelStyle`/`itemStyle` built from the same surface/border/text tokens every other card uses (`var(--color-surface)`, `var(--color-border)`, `rounded-md` equivalent, the Card box-shadow), `cursor` highlight (`bg-surface-secondary` fill for the two bar charts, a thin `border`-colored guide line for the area chart), and a `formatter` giving each chart a readable item label ("Researched" / "Jobs found" / "Jobs") instead of the raw `count` dataKey name.
- Deferred, not built this feature: the **incomplete profile banner** `project-overview.md`/`build-plan.md` mention for the dashboard. The delivered design screenshot doesn't show one (assumes a complete profile) and no feature 14-17 section specifies where its "is profile complete" check should come from — building it here would mean guessing a data source ahead of the features that actually wire real data. Flagged for whichever future feature adds real profile-completeness wiring to this page.

**Feature 15, Stats Bar — Real Data (built 2026-08-11):** `StatsBar` is now `{ stats: DashboardStats }`, no more hardcoded mock array. `app/(app)/dashboard/page.tsx` is now an `async` Server Component: auth-checks via `createInsforgeServer()` (`redirect("/login")` if no session, same pattern as `/find-jobs/[id]`), fetches every `jobs` row for the user (`match_score, company_research, found_at` only — the three columns the stats actually need) and runs it through `computeDashboardStats()` (new, `lib/dashboard.ts`, a pure function taking rows in and returning `DashboardStats` out — no DB access itself, same shape as `lib/profile.ts`'s mapper functions, keeping the actual query in the Server Component per `code-standards.md`).
- **Trend badges required a real definition build-plan's feature 15 bullet list didn't spec** (it only lists the 4 headline metrics, not how "vs last week" is computed) — since the design already committed to showing trend badges on Total Jobs Found and Avg. Match Rate, leaving them hardcoded would have been a real bug once real data landed, not a faithful "keep the mock". Defined: Total Jobs Found trend = relative % change in job count (this-week vs previous-week, i.e. day 0-7 vs day 7-14 ago); Avg. Match Rate trend = percentage-**point** difference in average `match_score` between the same two windows (not relative growth — a rate metric moving from 79% to 82% is "+3", not "+3.8%"). Both trends are `null` (badge hidden entirely, not shown as "+0%" or "N/A") whenever the prior week has no comparable data (zero jobs, or zero *scored* jobs) — division by zero / an undefined average would otherwise produce a meaningless number. Companies Researched and Jobs This Week never had a trend in the design and still don't.
- No red/negative-trend badge token exists anywhere in `ui-tokens.md`; since the design only shows positive examples, a decline still renders in the same green `success-lightest`/`success-darker` badge tokens with a `-` sign from `formatTrend()`, rather than inventing an undocumented error-colored variant.
- Verified against the real InsForge project, not just type-checked: ran the same aggregate query `computeDashboardStats()` implements directly via `insforge db query` against the real `dercio.anselmo@gmail.com` user's 56 real `jobs` rows (`count(*)`, `avg(match_score)`, `count(*) filter (where company_research is not null)`, week/prior-week windows) — 56 total, 71% avg, 4 researched, 56 this week, 0 in the prior week. Then screenshotted the actual rendered `StatsBar` (temporarily swapping the page's session-cookie client for an `@insforge/sdk` `createAdminClient()` call scoped to that same real user id, since a real OAuth session isn't automatable here — same precedent as feature 10/12's temporary-patch verification technique) and confirmed the four cards showed those exact numbers with both trend badges correctly absent (matches the `null`-on-no-prior-week-data rule, not a bug). Temporary page edit and the `proxy.ts` bypass were both reverted immediately after; `proxy.ts` confirmed byte-identical to the last commit again.
...
- Card: `rounded-2xl border border-border bg-surface p-6 shadow`
- Row: `grid gap-4 sm:grid-cols-[1fr_auto] sm:items-end` — Job Title (search icon prefix, `Input` `pl-9`), `Button variant="primary" className="h-10.5"` with a `Search` icon, label "Find Jobs" / "Searching..." while pending
- The Job Title input is **controlled** (`jobTitle` `useState`, `placeholder` unchanged from feature 09's exact copy). Enter key triggers the same search as clicking the button.
- **Location input removed (2026-08-13, feature 18 — `docs/specs/0002-emprego-job-discovery/index.md`).** The grid dropped from `[1fr_1fr_auto]` (Job Title / Location / Button) to `[1fr_auto]`; this is a real, deliberate design-spec change, not a regression — emprego.co.mz's keyword search and its location browsing don't combine (confirmed live during that spec's design), so there's no location filter for this source. If a future source restores location filtering, this grid and the removed `location` state/prop plumbing would need to come back.
...
- **Default sort changed from Match Score to Newest (2026-08-13), per direct user feedback** ("more useful to have the new searched on top"). `app/(app)/find-jobs/page.tsx`'s `parseSort()` now falls back to `"newest"` instead of `"matchScore"` when no `?sort=` param is present; the `<option>` order in the `Select` was also reordered (Newest, Oldest, Match Score) to match. This pairs with the same-day dedup fix below — re-searching an already-found job now bumps its `found_at`, so "newest first" is also "most recently re-confirmed first."
...
- **Superseded 2026-08-13 by feature 18 (`docs/specs/0002-emprego-job-discovery/index.md`) — see the section below.** `lib/adzuna.ts` and `agent/adzuna.ts` were deleted outright, not kept dormant; `agent/types.ts`'s `NormalizedAdzunaJob` was renamed `NormalizedJob`. `agent/matcher.ts` and `MATCH_THRESHOLD` are unaffected, still exactly as described above (still Azure OpenAI via `azureOpenai`/`AZURE_OPENAI_DEPLOYMENT`, not Claude — that provider note above was already stale before this migration, see `architecture.md`'s note). `app/api/agent/find/route.ts` is described fresh below, not incrementally.

### emprego.co.mz Job Discovery (feature 18, designed via `/architect`, built 2026-08-13)

- Full design record: `docs/specs/0002-emprego-job-discovery/index.md` (status `Proposed` → `In Progress` once build lands) — read it before touching any of the files below; it documents the load-bearing decisions (why a plain fetch + parse instead of Browserbase/Stagehand, the apply-flow decision, the page-scan/result caps, why Location left the search UI) that aren't repeated here.
- `lib/emprego.ts` — no InsForge/agent imports, pure fetch + `cheerio` parsing, verified against the real live site during the build (not assumed): `searchResultsUrl(jobTitle, page)` builds `/pesquisa/{slug}/` (words joined by `+`, matching the site's own redirect target, confirmed by following a real `?s=` redirect) or `/pesquisa/{slug}/page/{n}/`; `fetchSearchResultsPage()` returns `EmpregoSearchResult[]` (`title`, `companyName`, `location`, `detailUrl`, `isExpired`) or `null` on fetch failure — `isExpired` is a literal text match on the site's own "Expirado" label (confirmed both forms — the label and a real validity date like `17.08.2026` — against live pages, not guessed); `fetchJobDetail()` returns `{aboutRole, responsibilities, requirements}` or `null`, parsed from the job page's own `Funções`/`Requisitos` sections (matched by heading text, not tag level — the site uses a mix of `<h5>`/`<h6>` across postings, confirmed by comparing two real pages).
- `agent/types.ts` — `NormalizedJob` (renamed from `NormalizedAdzunaJob`, reshaped: `responsibilities`/`requirements` arrays added, `redirectUrl` renamed `sourceUrl`, `jobType` now `string | null`), `ScoredJob` unchanged in shape.
- `agent/matcher.ts` — `scoreJob(job, profile)` unchanged in structure (still a forced Azure OpenAI tool call to `record_match`) but the tool schema gained a `jobType` field (`"fulltime" | "parttime" | "contract" | null`, AC-6 of the spec — classified from the posting's own text since the site has no structured contract-type field, `null` when the text gives no real signal) and the user prompt now includes `responsibilities`/`requirements`, not just the title/company/location/description used against Adzuna's flatter shape.
  - **Temperature lowered 0.3 → 0.1 (2026-08-13), reported by the user as "same job, different match score, I didn't change my profile."** Expected LLM behavior (temperature 0.3 samples meaningfully), but confusing/untrustworthy for a scoring task specifically — the user should be able to trust that an unchanged job against an unchanged profile lands close to the same number. Not fully deterministic even at 0 (the model has some inherent variance regardless), but this tightens it meaningfully without needing a bigger change (e.g. caching scores) that wasn't asked for.
- `agent/emprego.ts` — `discoverJobs(insforge, jobTitle, profile, userId, runId)` (no `location` parameter, unlike the old Adzuna signature). `collectActiveResults()` scans up to 5 pages, keeping non-expired results, stopping at 10 — both caps are load-bearing (spec AC-1/AC-2: a broad keyword can span thousands of pages on this site, confirmed live during design). A page that parses to zero results on page 1 only (not later pages, where that's just "ran out of results") logs an `agent_logs` warning that the site's structure may have changed (AC-8). Each kept result's detail page is fetched for the structured description fields; a failed detail fetch still saves the job (title/company/location only, `about_role`/`responsibilities`/`requirements` left null) with a warning logged, never dropped (AC-4). `external_apply_url` is always null; `source_url` always the real emprego.co.mz job page (AC-7) — verified live that `JobActions.tsx`'s existing `job.external_apply_url ?? job.source_url` fallback needed no changes for this.
  - **Dedup on re-search added 2026-08-13, reported by the user as duplicate rows for the same job with no way to consolidate them.** The final `.insert(jobRows)` became `.upsert(jobRows, { onConflict: "user_id,source_url" })`, backed by a new `jobs_user_source_url_unique` constraint (`migrations/20260813030426_jobs-user-source-url-unique.sql`, a plain `UNIQUE (user_id, source_url)` — not a partial index, since standard SQL NULL semantics already let unlimited NULL-`source_url` rows coexist per user for free, and a plain constraint matches PostgREST's `onConflict` column-list target directly). `jobRows` deliberately never includes `company_research`, `id`, or `created_at` — confirmed (again; same precedent as feature 06's profile upsert) that PostgREST's `ON CONFLICT DO UPDATE` only touches columns present in the payload, so a job the user already ran company research on keeps that dossier across every future re-search instead of being silently wiped. Verified live: found and merged 4 real pre-existing duplicate rows for one job (one of which had a real `company_research` dossier — preserved during the merge), applied the migration, then ran the exact same search twice more against the real site — row count for that job stayed at 1 both times, `found_at` and `match_score` updated, `company_research` still intact.
- `app/api/agent/find/route.ts` — same shape as before (auth → zod body → profile check → `agent_runs` → PostHog `job_search_started` → `discoverJobs` → update run → `job_found` events → `revalidatePath` → response), but the zod schema dropped `location` entirely (not left optional/unused) and the zero-results message dropped "or location" from its copy.
- `components/find-jobs/SearchControls.tsx` — Location input and its `useState` removed; see its own entry above for the exact class change.
- **Real regression caught and fixed during the build, not part of the original spec**: `agent/research.ts`'s `deriveHomepageUrl()` (feature 13, company research) followed `job.source_url` expecting it to redirect through to the employer's real site, the way Adzuna's did. emprego.co.mz job pages don't redirect anywhere — they're the actual first-party posting — so unpatched, every future company research run for an emprego.co.mz-sourced job would have derived `emprego.co.mz` itself as the "company homepage" and researched the job board instead of the employer. Fixed by excluding `emprego.co.mz`'s own hostname the same way `adzuna.com`'s tracking domain already was, falling through to the existing company-name-guess fallback instead.
- Verified live end to end against the real InsForge project, not just type-checked: a scratch script ran the exact `discoverJobs` pipeline against the real site and the real `dercio.anselmo@gmail.com` profile for the query "gestor" — 4 real jobs saved (2 from WWF, one from Médicos Sem Fronteiras, one from Olam Agri), real Azure OpenAI match scores (0/20/0/40 — correctly low, the test profile is a software/cloud profile against non-tech roles), real `about_role`/`responsibilities`/`requirements` populated from each job's actual detail page, `external_apply_url` null and `source_url` set on every row, `job_type` classified `"fulltime"` on all four, zero `agent_logs` warnings. The real rows were left in the database (real data, not synthetic).
